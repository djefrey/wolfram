...
getOpts conf ("--rule":str:strs) = let mVal = readMaybe str :: Maybe Int in case mVal of
    Nothing -> Nothing
    (Just val) -> getOpts conf{rule=(Just val)} strs
getOpts conf ("--start":str:strs) = let mVal = readMaybe str :: Maybe Int in case mVal of
    Nothing -> Nothing
    (Just val) -> getOpts conf{start=val} strs
getOpts conf ("--lines":str:strs) = let mVal = readMaybe str :: Maybe Int in case mVal of
    Nothing -> Nothing
    (Just val) -> getOpts conf{line=(Just val)} strs
getOpts conf ("--window":str:strs) = let mVal = readMaybe str :: Maybe Int in case mVal of
    Nothing -> Nothing
    (Just val) -> getOpts conf{window=val} strs
getOpts conf ("--move":str:strs) = let mVal = readMaybe str :: Maybe Int in case mVal of
    Nothing -> Nothing
